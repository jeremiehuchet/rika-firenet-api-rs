...
/// struct for passing parameters to the method [`stove_contols`]
#[derive(Clone, Debug, Default)]
pub struct StoveContolsParams {
    /// Session cookie
    pub connect_period_sid: String,
    /// Stove identifier
    pub stove_id: String,
...
/// struct for typed errors of method [`stove_contols`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum StoveContolsError {
    Status404(String),
    UnknownValue(serde_json::Value),
}
...
pub async fn stove_contols(configuration: &configuration::Configuration, params: StoveContolsParams) -> Result<(), Error<StoveContolsError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<StoveContolsError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
